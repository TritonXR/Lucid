...
function Start()
{
	...
	GetComponent.<Animation>()["run"].speed = ForwardSpeed/3;
...
function Update () 
{
    ...
    if(charController.isGrounded == true)
    {
		...
		else if (Input.GetButton ("Jump")) GetComponent.<Animation>().CrossFade("spellCasting06-1");
		
		else {
			
			if(Input.GetAxis("Horizontal") && !Input.GetAxis("Vertical")) GetComponent.<Animation>().CrossFade("turn");
			else GetComponent.<Animation>().CrossFade("SpellIncantation06");
